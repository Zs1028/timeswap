...
class OfferHelpPage extends StatelessWidget {
  const OfferHelpPage({super.key});
  ...
  Widget build(BuildContext context) {
    // All OPEN OFFER services (for Services Available tab)
    final servicesAvailableQuery = FirebaseFirestore.instance
        .collection('services')
        .where('serviceType', isEqualTo: 'offer')        // ⭐ offer
    ...
    // Only MY OFFERINGS (for Your Offerings tab)
    final yourOffersQuery = FirebaseFirestore.instance
        .collection('services')
        .where('serviceType', isEqualTo: 'offer')        // ⭐ offer
          ...
          child: Column(
            children: [
              ...
              Expanded(
                child: TabBarView(
                  children: [
                    // 1) Services Available – hide my own offerings
                    _ServicesList(
                      query: servicesAvailableQuery,
                      hideOwn: true,
                    ),

                    // 2) Your Offerings – only my offerings + Add Offering button
                    _YourOfferingTab(query: yourOffersQuery),
                  ],
                ),
              ),
            ],
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          IconButton(
            onPressed: () async {
              // you can reuse Need Help filter later or create another
              final result = await Navigator.pushNamed(
                context,
                AppRoutes.needHelpFilter,
              );
              if (result != null) debugPrint('Offer filter result: $result');
            },
...
/// Rounded underline – same as Need Help
...
/// Your Offering tab: list + Add Offering button
class _YourOfferingTab extends StatelessWidget {
  final Query<Service> query;
  const _YourOfferingTab({required this.query});
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot<Service>>(
      ...
      builder: (context, snapshot) {
        ...
        if (hideOwn) {
          services = services
              .where((s) => s.requesterId != OfferHelpPage.currentUserId)
              .toList();
        }
...
/// Card UI – reuse same style, just prefix "Offer Help:"
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      RichText(
                        text: TextSpan(
                          text: 'Offer Help: ', // ⭐ different prefix
